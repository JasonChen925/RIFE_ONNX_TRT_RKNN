...
config.max_workspace_size = 1<<30  # config.max_workspace_size 设置最大工作空间大小为 1GB (用于 TensorRT 的临时存储，如卷积优化缓存)，分配GPU内存
profile = builder.create_optimization_profile()     #builder.create_optimization_profile()：创建优化配置文件，用于动态形状推理。

profile.set_shape('imgs', [1,6,1440,2560], [1,6,1440,2560], [1,6,1440,2560])  #设置输入的最小、优化和最大形状
