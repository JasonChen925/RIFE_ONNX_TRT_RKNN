...
class IFNet(nn.Module):
    def __init__(self):
        ...
        self.block1 = IFBlock(17, c=150)  # 13 + 4
    ...
    def forward(self, x, scale=[4, 2, 1], timestep=0.5):
        ...
        for i in range(3):
            if flow is not None:
                flow_d, mask_d = stu[i](torch.cat((img0, img1, warped_img0, warped_img1, mask), 1), flow, scale[i])
                flow = flow + flow_d
                mask = mask + mask_d
            else:
                flow, mask = stu[i](torch.cat((img0, img1), 1), None, scale[i])
        ...
        if gt is not None and gt.shape[1] == 3:
            ...
            flow_teacher, merged_teacher = None, None

        for i in range(3):
            merged[i] = merged[i][0] * mask_list[i] + merged[i][1] * (1 - mask_list[i])
            if gt is not None and gt.shape[1] == 3:
                loss_mask = ((merged[i] - gt).abs().mean(1, True) > (merged_teacher - gt).abs().mean(1, True) + 0.01).float().detach()
                loss_distill += (((flow_teacher.detach() - flow_list[i]) ** 2).mean(1, True).sqrt() * loss_mask).mean()
